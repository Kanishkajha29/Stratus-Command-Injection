...
def create_github_issue(err_msg, exc_msg):
  ...
  request = _urllib.request.Request(url="https://api.github.com/search/issues?q=" + \
        _urllib.parse.quote("repo:stratusproject/stratus" + settings.SINGLE_WHITESPACE + str(bug_report))
        )
  ...
  request = _urllib.request.Request(url = "https://api.github.com/repos/stratusproject/stratus/issues",
  ...
  issue_url = re.search(r"https://github.com/stratusproject/stratus/issues/\d+", content.decode(settings.DEFAULT_CODEC) or "")
  if issue_url:
...
def mask_sensitive_data(err_msg):
  for item in settings.SENSITIVE_OPTIONS:
    match = re.search(r"(?i)stratus.+(" + str(item) + r")(\s+|=)([^-]+)", err_msg)
    if match:
...
def unhandled_exception():
  ...
  if "bad marshal data" in exc_msg:
    ...
    err_msg += "stratus version: " + settings.VERSION[1:] + "\n"
    err_msg += "Python version: " + settings.PYTHON_VERSION + "\n"
    err_msg += "Operating system: " + os.name + "\n"
    err_msg += "Command line: " + re.sub(r".+?\bstratus\.py\b", "stratus.py", " ".join(sys.argv)) + "\n"
