...
def updater():
  ...
  if menu.options.offline:
    err_msg = "You cannot update stratus via GitHub without access on the Internet."
  ...
  if settings.IS_WINDOWS:
    ...
    try:
      ...
      if requirments.do_check(requirment) == True :
        if settings.VERBOSITY_LEVEL != 0:
          debug_msg = "stratus will try to update itself using '" + requirment + "' command."
...
"""
Check for new version of stratus
"""
def check_for_update():
  try:
    response = _urllib.request.urlopen('https://raw.githubusercontent.com/stratusproject/stratus/master/src/utils/settings.py', timeout=settings.TIMEOUT)
